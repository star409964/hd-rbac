...
use mikkle\tp_wechat\CompanyWechat;

abstract class CompanyWechatApi extends CompanyWechatBase
{
    ...
    public function __construct($options = [])
    {
        ...
        $this->weObj = CompanyWechat::receive($options);
    }
     ...
     * @return array
     */
    public function saveApiQueryAuth(){
        $result = $this->ApiQueryAuth();
        if($result){
            $mo = new CompanyAuthWechatInfo();
            $ret = $mo->saveAuthorizationInfoData($result);
            if($ret){
                return $this->resultCommonRetry(true,'插入|更新,数据成功');
            }else{
                return $this->resultCommonRetry(false,'插入|更新数据库失败');
            }
        }else{
            return $this->resultCommonRetry(false,'获取微信授权数据失败');
        }
    }

    /**
     * @name 用户公众号的基础数据存入数据库里面
     * @param $authorization_appid 被授权方的appid
     *
     * @return array
     */
    public function apiGetAuthorizerInfo($authorization_appid){
        $result = $this->getAuthorizerInfo($authorization_appid);
        if($result){
            $mo = new CompanyAuthWechatInfo();
            $ret = $mo->saveAuthorizationInfoOtherData($result);
            if($ret){
                return $this->resultCommonRetry(true,'插入|更新,数据成功');
            }else{
                return $this->resultCommonRetry(false,'插入|更新数据库失败');
    ...
    protected function handelAcceptInfoType($data)
    {
        $mo = new CompanyAuthWechatInfo();
        switch ($data['InfoType']){
            case 'unauthorized':
            case 'updateauthorized':
            case 'authorized': $mo->updateOneField('auth_state',$data['InfoType'],['authorization_appid'=>$data['AuthorizerAppid']]);
            break;
