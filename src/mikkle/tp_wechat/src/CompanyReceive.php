...
class CompanyReceive extends CompanyWechatBase
{
    ...
    /**
     * CompanyReceive constructor.
     *
     * @param array $authorization_appid 第三方平台的appid
     * @param array $options
     */
    public function __construct($authorization_appid, $options = [])
    {
        parent::__construct($options);
        $this->config['component_appid'] = $authorization_appid;
    }
    ...
    public function getComponentAppid()
    {
        if (isset($this->_receive['ToUserName'])) {
            ...
            $info = $mo->useUserNameGetInfo($this->_receive['ToUserName']);
            if (isset($info['authorization_appid'])) {
                return $info['authorization_appid'];
            } else {
                return false;
            }
        }
    ...
    public function reply($msg = array(), $return = false)
    {
        ...
        if ($this->encrypt_type == 'aes') { //如果来源消息为加密方式
            $xmldata = $this->msgDecrypt($xmldata);
        }
