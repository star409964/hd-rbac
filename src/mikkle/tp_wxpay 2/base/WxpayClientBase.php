...
abstract class WxpayClientBase
{
    ...
    public function postXml()
    {
        try{
            $xml = $this->createXml();
            Log::error( $xml );
            $this->response = Tools::postXmlCurl($xml,$this->url,20, $this->referer);
        }catch (Exception $e){
    ...
    public function postXmlSSL()
    {
        ...
        if ( $this->useCaPath ){
            ...
            $this->response = Tools::postXmlSSLCurl($xml,$this->url,$certPath,$keyPath,$this->caPath ,$this->curlTimeout,$this->referer);
        }else{
            $this->response = Tools::postXmlSSLCurl($xml,$this->url,$certPath,$keyPath,false,$this->curlTimeout,$this->referer);
        };
        return $this->response;
    }
